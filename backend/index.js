...
const getLocalIPAddress = () => {
  ...
  return "102.176.94.215"; // Fallback to 102.176.94.215 if no address is found
};
...
app.use(cors({
  origin: "http://102.176.94.215:5173", // your frontend port
...
const formattedAudio = audioPosts.map((audio) => {
  ...
  return {
   ...
   const userProfile = userResult.rows[0]
    ...
    res.redirect("http://102.176.94.215:5173/login");
  }
});
    ...
    if (checkResult.rows.length > 0) {
      ...
      bcrypt.hash(password, saltRounds, async (err, hash) => {
        if (err) {
          ...
          req.login(user, (err) => {
            console.log(err);
            res.redirect("http://102.176.94.215:5000/feeds");
          });
        }
      });
    }
...
app.post("/login", (req, res, next) => {
    passport.authenticate("local", (err, user, info) => {
      ...
      req.login(user, (err) => {
        ...
        return res.json({ redirect: "http://102.176.94.215:5000/feeds" });
      });
    })(req, res, next);
  });
